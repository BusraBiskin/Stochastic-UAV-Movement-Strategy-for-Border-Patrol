...
Result = np.zeros((100*n, 5))
Result[0:4,] = V[0:4,]
Result_best = np.zeros((100*n, 5))
...
while elapsed < 400:
        ...
        if accept == 0:
            ...
            while m != 10 and cr1 != 5 :
                ...
                if r < P_s:
                    ...
                    N = N[0:(n+1)]
                    X_ARS = NeighbourhoodLS(N, number_of_runs, ratio, l, h, max_number_of_steps, Dist_Matrix_1, Dist_Matrix_2, Dist_Matrix_3,radius)
    ...
    resampled_best = np.zeros((4,7))
    #resampling V_sort
    for j in range(4):
        resampled = np.zeros((5,3))
        for i in range(5):
            resampled[i,0:3] = Statistics_of_Simulation(V_sort[j,0:4], 1500, ratio, l, h, max_number_of_steps, Dist_Matrix_1,Dist_Matrix_2,Dist_Matrix_3, radius)[0:3]
        resampled_best[j,0:4] = V_sort[j,0:4]
...
best_soln = best_soln_sort[3,0:7]
