...
Perf = Statistics_of_Simulation(P, number_of_runs, ratio, l, h, max_number_of_steps, Dist_Matrix_1,Dist_Matrix_2,Dist_Matrix_3,radius)
f = Perf[2]
f_best = Perf[2]
P_best = P

Result = np.zeros((10000*n, 7))
...
while elapsed < 400:
    ...
    for j in range(n):
        ...
        Perf_1 = Statistics_of_Simulation(P_1, number_of_runs, ratio, l, h, max_number_of_steps, Dist_Matrix_1,Dist_Matrix_2,Dist_Matrix_3, radius)
        f_1 = Perf_1[2]
...
resampled_best = np.zeros((5,7))
    
#resampling first 5
for j in range(5):
    resampled = np.zeros((5,3))
    for i in range(5): 
        resampled[i,0:3] = Statistics_of_Simulation(Result_sorted_5[j,0:4], 1500, ratio, l, h, max_number_of_steps, Dist_Matrix_1,Dist_Matrix_2,Dist_Matrix_3, radius)[0:3]
    resampled_best[j,0:4] = Result_sorted_5[j,0:4]
    resampled_best[j,4] = np.average(resampled[0:4,0])
    resampled_best[j,5] = np.average(resampled[0:4,1])
    resampled_best[j,6] = np.average(resampled[0:4,2])
    
"best solution"
best_soln_sort = resampled_best[(resampled_best[:,6]).argsort()]
best_soln = best_soln_sort[4,0:7] 

Statistics_of_Simulation(best_soln[0:4],1500, ratio, l, h, max_number_of_steps, Dist_Matrix_1,Dist_Matrix_2,Dist_Matrix_3, radius)   

#Result = Result[0:8174,]

#xpoints = np.array(range(1,8175))
#ypoints = np.array(Result[0:8174,6])
